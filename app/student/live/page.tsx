...
export default function StudentLivePage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div>
        <h2 className="text-3xl font-black text-slate-800">Live Sessions</h2>
        <p className="text-slate-500 mt-1">Live and recorded lectures</p>
      </div>
      ...
      {displayed.length === 0 ? (
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm text-center py-20">
          ...
          <p className="text-slate-400">{tab === 'upcoming' ? 'No upcoming sessions' : 'No past sessions'}</p>
        </div>
      ) : (
                ...
                <div className="flex gap-2">
                  ...
                  {s.isRecorded && s.recordingUrl && (
                    <a href={s.recordingUrl} target="_blank" rel="noopener noreferrer"
                      ...
                      <MonitorPlay size={14} /> Watch Recording
                    </a>
                  )}
                </div>
              </div>
