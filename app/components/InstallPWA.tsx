...
export function InstallPWAButton({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const ios = /iphone|ipad|ipod/i.test(navigator.userAgent) && !(window as any).MSStream;
    setIsIOS(ios);
    if (ios) return; // button hidden on iOS — banner handles it
  ...
  return (
    <>
      ...
      {showIOSGuide && (
        <div className="fixed inset-0 z-[110] flex items-end justify-center p-4 bg-black/50" onClick={() => setShowIOSGuide(false)}>
          <div className="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-sm p-6" onClick={e => e.stopPropagation()}>
            <p className="font-black text-center mb-3">Install on iPhone</p>
            <p className="text-sm text-slate-500 text-center mb-4">Tap Share (□↑) → "Add to Home Screen" → Add</p>
            <button onClick={() => setShowIOSGuide(false)} className="w-full py-3 bg-indigo-600 text-white font-bold rounded-2xl">Got it</button>
          </div>
        </div>
      )}
    </>
